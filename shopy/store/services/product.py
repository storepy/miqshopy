...
from shopy.store.models import Product,ProductImage
...
def product_create(*, name: str, retail_price: Decimal, slug: Optional[str] = None, commit: bool = True, **kwargs) -> Product:
    """Create a product."""
...
def product_delete(product: Product) -> None:
    ...
    page = product.page
    if (attrs := product.attributes) and attrs.exists():
        attrs.all().delete()

    id = product.id
    product.delete()

    imgs = ProductImage.objects.filter(product__id=id)
    if imgs.exists():
        imgs.delete()
    ...
    # if (imgs := product.images) and imgs.exists():
    #     imgs.all().delete()
